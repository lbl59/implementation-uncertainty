...
Plots the pathways for the original SP compromise and its pathways when Watertown and Dryville 
experience their individual worst robustness.
...
Plotting for Fallsland occurs separately due to differing number of clusters
...
def plot_single_pathway(cluster_medians, cluster_pathways, inf_options_idx,
    ...
    ax.imshow((inf_im.T/0.85), cmap=cmap, aspect='auto', alpha = 0.60)
...
for i in range(len(rdm_idx)):
    ...
    fig_fileloc = fig_filepath + compSol_full + title_label + '_pathways_indv_cbar.pdf'                                                                                                                                     
    
    fig = plt.figure(figsize=(15,13), dpi=300)
    gspec = fig.add_gridspec(nrows=3, ncols=4, height_ratios =[1,1,1])
    
    for r in range(len(soln_idxs)-1):
        solution = soln_idxs[r]    #'Drought + High demand scenario'
        mode = soln_modes[r]   
        ...
        create_cluster_plots(watertown_cluster_meds, dryville_cluster_meds,
                             ...
                             ['bone_r', 'bone_r', 'bone_r'], fig, gspec, r, False)
